...
const chartMaxValue = computed(() =>
    Math.max(
        0,
        ...props.chartSeries.flatMap((series) => series.values),
    ),
);

const chartInnerWidth =
    chartDimensions.width - chartDimensions.paddingLeft - chartDimensions.paddingRight;
const chartInnerHeight =
    chartDimensions.height - chartDimensions.paddingTop - chartDimensions.paddingBottom;
...
<template>
            ...
            <Button size="sm" @click="openCreateSnapshot()">
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        >
                            Ko bo shranjen prvi mesečni povzetek, se bo tukaj prikazal graf.
                        </div>
                ...
                <Card>
                    ...
                    <CardContent>
                        ...
                        <Table v-else>
                            <thead>
                                <tr class="border-b">
                                    <th class="h-10 px-2 text-left font-medium whitespace-nowrap">Mesec</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Varčevanje</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Obveznice</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">ETF</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Kripto</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Delnice</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Skupaj</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">% Diff</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Diff</th>
                                    <th class="h-10 px-2 text-left font-medium whitespace-nowrap">Vir</th>
                                    <th class="h-10 px-2 text-right font-medium whitespace-nowrap">Akcije</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr
                                ...
                                >
                                    <td class="p-2 align-middle whitespace-nowrap">{{ row.month_label }}</td>
                                    <td class="p-2 text-right align-middle whitespace-nowrap">{{ formatMoney(row.savings_amount) }}</td>
                                    <td class="p-2 text-right align-middle whitespace-nowrap">{{ formatMoney(row.bond_amount) }}</td>
                                    <td class="p-2 text-right align-middle whitespace-nowrap">{{ formatMoney(row.etf_amount) }}</td>
                                    <td class="p-2 text-right align-middle whitespace-nowrap">{{ formatMoney(row.crypto_amount) }}</td>
                                    <td class="p-2 text-right align-middle whitespace-nowrap">{{ formatMoney(row.stock_amount) }}</td>
                                    <td class="p-2 text-right align-middle whitespace-nowrap font-semibold">{{ formatMoney(row.total_amount) }}</td>
                                    <td
                                    ...
                                    <td class="p-2 align-middle whitespace-nowrap">
                                        <Badge :variant="row.source === 'scheduled' ? 'secondary' : 'outline'">
                                            {{ row.source_label }}
                                        </Badge>
                                    </td>
    ...
    <Dialog :open="showSnapshotModal" @update:open="showSnapshotModal = $event">
        <DialogContent class="sm:max-w-xl">
            <DialogHeader>
                <DialogTitle>
                    {{ editingSnapshot === null ? 'Dodaj mesečni povzetek' : 'Uredi mesečni povzetek' }}
                </DialogTitle>
                <DialogDescription>
                    Vnesi zgodovinske vrednosti za izbrani mesec. Skupaj se izračuna samodejno.
                </DialogDescription>
            </DialogHeader>
            ...
            <form class="grid gap-4 md:grid-cols-2" @submit.prevent="submitSnapshot">
                <div class="grid gap-2 md:col-span-2">
                    ...
                    <Input id="month_date" v-model="snapshotForm.month_date" type="month" />
                    <InputError :message="snapshotForm.errors.month_date" />
                </div>

                <div class="grid gap-2">
                    <Label for="savings_amount">Varčevanje</Label>
                    <Input id="savings_amount" v-model="snapshotForm.savings_amount" type="number" min="0" step="0.01" />
                    <InputError :message="snapshotForm.errors.savings_amount" />
                </div>

                <div class="grid gap-2">
                    <Label for="bond_amount">Obveznice</Label>
                    <Input id="bond_amount" v-model="snapshotForm.bond_amount" type="number" min="0" step="0.01" />
                    <InputError :message="snapshotForm.errors.bond_amount" />
                </div>

                <div class="grid gap-2">
                    <Label for="etf_amount">ETF</Label>
                    <Input id="etf_amount" v-model="snapshotForm.etf_amount" type="number" min="0" step="0.01" />
                    <InputError :message="snapshotForm.errors.etf_amount" />
                </div>

                <div class="grid gap-2">
                    <Label for="crypto_amount">Kripto</Label>
                    <Input id="crypto_amount" v-model="snapshotForm.crypto_amount" type="number" min="0" step="0.01" />
                    <InputError :message="snapshotForm.errors.crypto_amount" />
                </div>

                <div class="grid gap-2">
                    <Label for="stock_amount">Delnice</Label>
                    <Input id="stock_amount" v-model="snapshotForm.stock_amount" type="number" min="0" step="0.01" />
                    <InputError :message="snapshotForm.errors.stock_amount" />
                </div>

                <div class="rounded-lg border border-dashed bg-muted/20 p-4 md:col-span-2">
                    <p class="text-sm text-muted-foreground">
                        Predviden skupaj:
                            ...
                            {{
                                formatMoney(
                                    Number(snapshotForm.savings_amount || 0) +
                                        ...
                                        Number(snapshotForm.crypto_amount || 0) +
                ...
                <DialogFooter class="md:col-span-2">
                    <Button type="button" variant="outline" @click="showSnapshotModal = false">
                        Prekliči
                    </Button>
                    <Button type="submit" :disabled="snapshotForm.processing">
                        {{ editingSnapshot === null ? 'Shrani mesec' : 'Posodobi mesec' }}
                    </Button>
                </DialogFooter>
            </form>
        </DialogContent>
